...
Launch three review passes in parallel as subagents when the platform supports concurrent subagent dispatch; otherwise run them sequentially in this order. Give each pass the full diff so it has complete context.
...
Aggregate the findings from all three passes and fix each issue directly. If a finding is a false positive or not worth addressing, note it and move on — do not argue with the finding, just skip it.
